...
t.test('LsmInterpolator', (t) => {
	...
	t.test('with specific coeffs count', (t) => {
		const interpolator = LsmInterpolator.fit({xs, ys, coeffsCount: 2});
	...
	t.test('with autofit coeffs count', (t) => {
		const interpolator = LsmInterpolator
			.fit({xs, ys, coeffsCount: {min: 2, max: 10}});
